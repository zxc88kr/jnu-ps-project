...
void solve(std::vector<int>& towers, int n)
{
	...
	{
		int targetTowerIndex = 0; // 레이저 신호를 수신한 탑이 존재하지 않는다면 기본적으로 0을 출력

		// 현재 탑의 높이보다 낮은 탑들은 레이저를 수신하지 못하므로, 이에 해당하는 탑들을 전부 스택에서 pop
		...
		stackHeight.push(towers[i]); // 현재 탑의 높이를 스택에 push
		stackIndex.push(i + 1); // 현재 탑의 인덱스 번호를 스택에 push, 실제 인덱스는 1부터 시작
	}
	printf("\n");
}
