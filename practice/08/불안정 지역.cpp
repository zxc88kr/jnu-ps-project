 ...
 * @param cities 도시의 구조체 벡터 cities[0] ~ cities[n - 1]
 * @param n      도시의 수
 * @param k      연속한 도시의 수
 * @return       연속한 k개의 도시로 이루어진 영역들 중 가장 큰 소득차를 가지는 영역의 소득차를 반환
 */
int getMaxDifferenceInRange(std::vector<City>& cities, int n, int k)
{
    ...
    for (int i = 0; i < k - 1; i++) // k - 1 개의 도시 정보를 우선순위 큐에 push
    {
        maximumInRange.push(cities[i]);
        minimumInRange.push(cities[i]);
    }
    for (int i = k - 1; i < n; i++) // 나머지 도시 정보를 정해진 규칙에 따라 우선순위 큐에 push
    {
        maximumInRange.push(cities[i]);
        minimumInRange.push(cities[i]);
        ...
        // 현재 영억에서의 소득차를 기준으로 가장 큰 소득차 값을 업데이트
...
int main()
{
	...
	for (int idx = 0; idx < t; idx++)
	{
        ...
        for (int i = 0; i < n; i++)
        {
            // 입력받은 소득 수준을 바탕으로 0번째 도시의 구조체부터 생성
            int income;
            scanf("%d", &income);
